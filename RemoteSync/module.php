...
class RemoteSync extends IPSModule
{
    ...
    private function InitConnection()
    {
        ...
        try {
            ...
            $url = $serverConfig['URL'] ?? $serverConfig['url'] ?? $serverConfig['Url'] ?? null;
            $user = $serverConfig['User'] ?? $serverConfig['user'] ?? $serverConfig['Username'] ?? null;
            $pw = $serverConfig['PW'] ?? $serverConfig['pw'] ?? $serverConfig['Password'] ?? null;
    ...
    private function GenerateActionScriptCode($localID, $remSecID, $locKey)
    {
        ...
        return "<?php
/* Auto-Generated by RemoteSync Module (Try/Fallback Version) */
\$targetID = $localID;
\$secID = $remSecID;
\$key = '$locKeySafe'; 
...
try {
    ...
    // If error is -32603 (No valid action), fallback to SetValue
    if (\$e->getCode() == -32603) {
